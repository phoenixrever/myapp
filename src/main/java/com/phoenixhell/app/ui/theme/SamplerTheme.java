package com.phoenixhell.app.ui.theme;

/* SPDX-License-Identifier: MIT */

import static com.phoenixhell.app.Launcher.IS_DEV_MODE;
import static com.phoenixhell.app.util.Resources.resolve;
import static com.phoenixhell.app.ui.theme.ThemeManager.APP_STYLESHEETS;
import static com.phoenixhell.app.ui.theme.ThemeManager.DUMMY_STYLESHEET;
import static com.phoenixhell.app.ui.theme.ThemeManager.PROJECT_THEMES;
import static java.nio.charset.StandardCharsets.UTF_8;
import static java.nio.file.LinkOption.NOFOLLOW_LINKS;

import atlantafx.base.theme.Theme;
import com.phoenixhell.app.FileResource;
 ...
 * <br/>
 * <br/>
 * JavaFX doesn't provide the theme API. Moreover, there is no such notion, it's
 * called
 * "user agent stylesheet". There are two ways to change the platform
 * stylesheet:
 * <ul>
 * <li>{@link Scene#setUserAgentStylesheet(String)}</li>
 * <li>{@link Application#setUserAgentStylesheet(String)}</li>
 * </ul>
 * The first one is observable and the latter is not. Both fail to provide hot
 * reload with
 * {@link CSSFX}. This means that if we'd set up some CSS file as the platform
 * stylesheet
 * and then change (re-compile) this file, nothing will happen.
 * <br/>
 * <br/>
 * To work around this issue we use dev mode trick. In this mode, we set the
 * dummy stylesheet
 * to the Application, while real theme CSS is added to the
 * {@link Scene#getStylesheets()}
 * along with app own stylesheet. When CSSFX detects any of those CSS files
 * changes, it forces
 * observable list update and thus reloading CSS with changes.
 * <br/>
 * <br/>
 * Also, note that some styles aren't applied when using
 * {@link Application#setUserAgentStylesheet(String)}.
 * E.g. JavaFX ignores Ikonli -fx-icon-color and -fx-icon-size properties, but
 * for an unknown
 * reason they won't be ignored when exactly the same stylesheet is set via
 * {@link Scene#getStylesheets()}.
 * </p>
 */
public final class SamplerTheme implements Theme {
    ...
    private static final Pattern COLOR_PATTERN = Pattern
            .compile("\s*?(-color-(fg|bg|accent|success|danger|warning)-.+?):\s*?(.+?);");
    ...
    // Application.setUserAgentStylesheet() only accepts URL (or URL string
    // representation),
    ...
    // Tries to parse theme CSS and extract conventional looked-up colors. There are
    // few limitations:
