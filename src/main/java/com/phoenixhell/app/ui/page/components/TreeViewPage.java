...
public final class TreeViewPage extends OutlinePage {
    ...
    private VBox playground() {
        ...
        denseToggle.selectedProperty().addListener(
                (obs, old, value) -> findDisplayedTree().ifPresent(tv -> Styles.toggleStyleClass(tv, Styles.DENSE)));
        ...
        altIconToggle.selectedProperty().addListener(
                (obs, old, val) -> findDisplayedTree().ifPresent(tv -> Styles.toggleStyleClass(tv, Tweaks.ALT_ICON)));

        var edge2edgeToggle = new ToggleSwitch("Edge to edge");
        edge2edgeToggle.selectedProperty().addListener((obs, old, val) -> findDisplayedTree()
                .ifPresent(tv -> Styles.toggleStyleClass(tv, Tweaks.EDGE_TO_EDGE)));
